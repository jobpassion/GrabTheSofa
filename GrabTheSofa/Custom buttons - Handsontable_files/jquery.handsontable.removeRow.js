...
$('document').ready(function(){
			...
			colWidths: [55, 80, 80, 80, 80, 80, 80],
				  ...
				  scrollV: 'auto',
				  scrollH: 'auto',
				  colHeaders: ["启用", "标题", "Url", "regexp(不懂请保持默认)"],
			  ...
			  if(!_init)
			  return;
				var _data = container.data('handsontable').getData();
				console.log(_data);
				// var _jobs = [];
				// for(var i=0; i<_data.length; i++){
					// if(null == _data[i][0] || "" == _data[i][0] ||null == _data[i][1] || "" == _data[i][1] ||null == _data[i][2] || "" == _data[i][2] ||null == _data[i][3] || "" == _data[i][3])
						// continue;
					// _jobs.push({url:_data[i][2], regexp:_data[i][3], title:_data[i][1], active:_data[i][0]});
					//$('#msg').html('saved. ')
				// }
				//alert(_data.length);
			  ...
			  columns: [
				...
				{data: "url", type: 'text'},
				{data: "regexp", type: {editor: Handsontable.AutocompleteEditor},source:['<tbody id=\\"normalthread.*\\s*<tr>\\s*.*\\s*.*\\s*.*\\s*.*\\s*.*\\s*<th class="(common|new)">\\s*.*(<a.*>.*</a>)', '()threadlist_text.*\\s*(<a.*</a>)']
